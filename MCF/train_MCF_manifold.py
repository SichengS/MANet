...
parser.add_argument('--exp', type=str,  default="edge", help='model_name')                               # todo model name
...
snapshot_path = "../model/" + args.exp + "_" + str(args.label_num)+ "/"
...
def get_edge_loss(v_edge, v_label, r_edge, r_label):
    edgev_lab = get_sobel_edge(v_label)
    edger_lab = get_sobel_edge(r_label)

    v_edge = torch.squeeze(v_edge).cpu()
    r_edge = torch.squeeze(r_edge).cpu()

    v_edge_loss = F.cross_entropy(v_edge, edgev_lab.long())
    r_edge_loss = F.cross_entropy(r_edge, edger_lab.long())
    
    return v_edge_loss, r_edge_loss
...
if __name__ == "__main__":
    ...
    labeled_idxs = list(range(args.label_num))           # todo set labeled num
    unlabeled_idxs = list(range(args.label_num, 80))     # todo set labeled num all_sample_num
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            v_outputs, v_edge = model_vnet(v_input)
            r_outputs, r_edge = model_resnet(r_input)
            ...
            v_edge_loss, r_edge_loss = get_edge_loss(v_edge[labeled_bs:], v_label[labeled_bs:], r_edge[labeled_bs:], r_label[labeled_bs:])
            ...
            v_supervised_loss =  ((v_loss_seg + v_loss_seg_dice) + 0.5 * v_mse) * (1-args.alpha) + v_edge_loss * args.alpha
            r_supervised_loss =  ((r_loss_seg + r_loss_seg_dice) + 0.5 * r_mse) * (1-args.alpha) + r_edge_loss * args.alpha
            ...
            logging.info(
                'iteration ： %d v_supervised_loss : %f v_loss_seg : %f v_loss_seg_dice : %f v_loss_mse : %f r_supervised_loss : %f r_loss_seg : %f r_loss_seg_dice : %f r_loss_mse : %f r_edge_loss : %f v_edge_loss : %f Good_student: %f'  %
                (iter_num,
                 v_supervised_loss.item(), v_loss_seg.item(), v_loss_seg_dice.item(), v_mse.item(),
                 r_supervised_loss.item(), r_loss_seg.item(), r_loss_seg_dice.item(), r_mse.item(), r_edge_loss.item(), v_edge_loss.item(), Good_student))
