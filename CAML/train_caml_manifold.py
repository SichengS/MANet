...
parser.add_argument('--model', type=str, default='caml3d_edge', help='model_name')
...
parser.add_argument('--alpha', type=float, default=0.01, help='weight to balance edge loss')

args = parser.parse_args()

snapshot_path = "./model/LA_{}_{}_memory{}_feat{}_labeled_numfiltered_{}_consistency_{}_rampup_{}_consis_o_{}_iter_{}_seed_{}/{}_{}".format(
    args.exp,
    args.labelnum,
    args.memory_num,
    args.embedding_dim,
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            model.train()
            outputs_v, outputs_a, embedding_v, embedding_a, edge_v, edge_a = model(volume_batch)
            outputs_list = [outputs_v, outputs_a]
            edge_list = [edge_v, edge_a]
            ...
            edge_ori = torch.zeros((num_outputs,) + edge_list[0].shape)
            edge_pseudo_label = torch.zeros((num_outputs,) + edge_list[0].shape)
            ...
            for i in range(num_outputs):
                ...
                edge = edge_list[i]
                edge_prob = F.softmax(edge, dim=1)
                edge_ori[i] = edge_prob
                edge_pseudo_label[i] = sharpening(edge_prob)
            ...
            for i in range(num_outputs):
                for j in range(num_outputs):
                    if i != j:
                        ...
                        loss_edge += consistency_criterion(edge_ori[i], edge_pseudo_label[j])
            ...
            loss = (args.lamda * loss_s + lambda_c * loss_c + lambda_o * loss_o) * (1-args.alpha) + loss_edge * args.alpha

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss : %03f, loss_s: %03f, loss_c: %03f, loss_o: %03f, loss_edge: %03f' % (
                iter_num, loss, loss_s, loss_c, loss_o, loss_edge))
            ...
            writer.add_scalar('edge_loss', loss_edge, iter_num)
