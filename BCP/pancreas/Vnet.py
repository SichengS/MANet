...
class VNet_Edge(nn.Module):
    def __init__(self, n_channels=1, n_classes=2, n_filters=16, normalization='instancenorm', has_dropout=False):
        ...
        self.Decoder = Decoder(n_classes=2, n_filters=16, normalization=normalization)
        self.Decoder_edge = Decoder(n_classes=2, n_filters=16, normalization=normalization)
    ...
    def forward(self, input, turnoff_drop=False):
        ...
        features = self.encoder(input)
        out = self.Decoder(features)
        edge = self.Decoder_edge(features)
        if turnoff_drop:
            self.has_dropout = has_dropout
        return out, edge