...
from Vnet import VNet, VNet_Edge
from torch.utils.data import Dataset
from torchvision.transforms import Compose


def create_Vnet(ema=False, edge = False):
    if edge:
        net = VNet_Edge()
    else:
...
def get_dataset_path(dataset='pancreas', labelp='20percent'):
    ...
    return ['/'.join(['./pancreas_data', f]) for f in files]
...
def get_ema_model_and_dataloader(data_root, split_name, batch_size, lr, labelp=10, edge = False):
    print("Initialize ema cutmix: network, optimizer and datasets...")
    """Net & optimizer"""
    net = create_Vnet(edge = edge)
    ema_net = create_Vnet(edge = edge, ema=True).cuda()
