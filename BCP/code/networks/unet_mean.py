...
class EdgeBlock(nn.Module):
    ...
    def forward(self, x):
        # x = F.softmax(x, dim = 1)
        x_mean = self.avg_pooling(x)
        x_mean = self.up_sample(x_mean)
        x = x-x_mean
        return self.conv_conv(x), x
...
class Decoder_Edge(nn.Module):
    def __init__(self, params):
        ...
        self.out_conv = nn.Conv2d(self.ft_chns[0], self.n_class, kernel_size=3, padding=1)
        self.Edge = EdgeBlock(self.n_class, 2)
    ...
    def forward(self, feature):
        ...
        x_last = self.up4(x, x0)
        output = self.out_conv(x_last)
        edge, x = self.Edge(output)
        return output, edge, x
...
class UNet_edge(nn.Module):
    def __init__(self, in_chns, class_num):
        ...
        self.encoder = Encoder(params)
        self.decoder = Decoder_Edge(params)
    ...
    def forward(self, x):
        feature = self.encoder(x)
        output, edge, x = self.decoder(feature)
        return output, edge, x
