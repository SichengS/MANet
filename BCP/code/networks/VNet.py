...
class Decoder_edge(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        ...
        self.dropout = nn.Dropout3d(p=0.5, inplace=False)
        # self.block_ten = EdgeBlock(n_classes, n_classes, 2, normalization=normalization)
    ...
    def forward(self, features):
        ...
        x8_up = self.block_eight_up(x2)
        x8_up = x8_up + x1
        x9 = self.block_nine(x8_up)

        if self.has_dropout:
            x9 = self.dropout(x9)
        edge = self.out_conv(x9)
        # edge = self.block_ten(out_seg)
        return edge, x8_up
...
class VNet_edge(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        super(VNet_edge, self).__init__()
        ...
        self.decoder_edge = Decoder_edge(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual)
        dim_in = 16
        feat_dim = 32
    ...
    def forward(self, input):
        ...
        edge, _ = self.decoder_edge(features)
        #features = self.pool(features[4])
        return out_seg, edge # 4, 16, 112, 112, 80
