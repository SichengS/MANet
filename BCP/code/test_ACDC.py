...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        if ind == 1:
            ...
            with torch.no_grad():
                out_main, edge = net(input)
                torch.set_printoptions(sys.maxsize)
                if len(out_main)>1:
                    out_main=out_main[0]
...
def Inference(FLAGS):
    ...
    snapshot_path = "./ACDC_result_{}_encoder/pretrain_{}_{}/self_train_{}_{}".format(FLAGS.alpha, FLAGS.labelnum, FLAGS.pretrain_model, FLAGS.model, FLAGS.alpha)
    test_save_path = "./ACDC_result_{}_encoder/pretrain_{}_{}/self_train_{}_{}/prediction/".format(FLAGS.alpha, FLAGS.labelnum, FLAGS.pretrain_model, FLAGS.model, FLAGS.alpha)
