...
from networks.net_factory import BCP_edge, net_factory
...
parser.add_argument('--pretrain_model', type=str, default='unet', help='pretrain_model_name')
parser.add_argument('--alpha', type=float, default=0.01, help='weight of original loss is (1-alpha), new loss is alpha')

args = parser.parse_args()

dice_loss = losses.DiceLoss(n_classes=4)
dice_loss_edge = losses.DiceLoss(n_classes=2)
...
def get_canny_edge(img):
    ...
    edge = np.empty_like(img)

    for i in range(len(img)):
        edge[i] = feature.canny(img[i], sigma = 3)
...
def pre_train(args, snapshot_path):
    ...
    model = BCP_edge(in_chns=1, class_num=num_classes)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # Find Edge
            edge_lab = get_canny_edge(gt_mixl)
            edge_lab = torch.tensor(np.array(edge_lab))
            edge_lab = edge_lab.long()
            #-- original
            net_input = img_a * img_mask + img_b * (1 - img_mask)
            out_mixl, edge = model(net_input)
            loss_edge = F.cross_entropy(edge.cpu(), edge_lab.long())
            loss_dice, loss_ce = mix_loss(out_mixl, lab_a, lab_b, loss_mask, u_weight=1.0, unlab=True)       

            loss = ((loss_dice + loss_ce) / 2) * (1-args.alpha) + loss_edge * args.alpha
            ...
            logging.info('iteration %d: loss: %f, mix_dice: %f, mix_ce: %f, loss_dice: %f, loss_edge: %f'%(iter_num, loss, loss_dice, loss_ce, loss_dice, loss_edge))
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    model = BCP_edge(in_chns=1, class_num=num_classes)
    ema_model = BCP_edge(in_chns=1, class_num=num_classes, ema=True)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            out_unl, edge_u = model(net_input_unl)
            out_l, edge_l = model(net_input_l)

            edgel_lab = get_canny_edge(l_label)
            edgeu_lab = get_canny_edge(unl_label)
            edgel_lab = torch.tensor(np.array(edgel_lab))
            edgeu_lab = torch.tensor(np.array(edgeu_lab))

            loss_edge = F.cross_entropy(edge_l.cpu(), edgel_lab.long()) + F.cross_entropy(edge_u.cpu(), edgeu_lab.long())
            ...
            logging.info('iteration %d: loss: %f, mix_dice: %f, mix_ce: %f, loss_edge: %f'%(iter_num, loss, loss_dice, loss_ce, loss_edge))
...
if __name__ == "__main__":
    ...
    # -- path to save models
    pre_snapshot_path = "./ACDC_result_{}_encoder/pretrain_{}_{}/pretrain".format(args.alpha, args.labelnum, args.pretrain_model)
    self_snapshot_path = "./ACDC_result_{}_encoder/pretrain_{}_{}/self_train_{}_{}".format(args.alpha, args.labelnum, args.pretrain_model, args.model, args.alpha)
