...
parser.add_argument('--pretrain_model', type=str, default='VNet', help='pretrain_model_name')
...
def l1_loss(out, gt):
    ...
    for i in range(len(out)):
        loss += nn.functional.l1_loss(out[i], gt[i])
    return loss
...
def pre_train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # Find label edge
            edge_lab = get_sobel_edge(label_batch)
            outputs, edge = model(volume_batch)

            loss_edge = F.cross_entropy(edge.cpu(), edge_lab.long(), weight = torch.Tensor([0.9,0.1]))
            loss_ce = F.cross_entropy(outputs, label_batch)
            loss_dice = DICE(outputs, label_batch)
            loss = ((loss_ce + loss_dice) / 2) * (1-args.alpha) + loss_edge * args.alpha
            ...
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss: %03f, loss_dice: %03f, loss_ce: %03f, loss_edge: %03f'%(iter_num, loss, loss_dice, loss_ce, loss_edge))
...
def self_train(args, pre_snapshot_path, self_snapshot_path):
    ...
    pretrained_model = os.path.join(pre_snapshot_path, f'{args.pretrain_model}_best_model.pth')
    ...
    for epoch in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # Return both output and edge from student model
            outputs_l, edge_l = model(mixl_img)
            outputs_u, edge_u = model(mixu_img)
            
            # Label edge
            edgel_lab = get_sobel_edge(mixl_lab)
            edgeu_lab = get_sobel_edge(mixu_lab)
            ...
            loss_edge = F.cross_entropy(edge_l.cpu(), edgel_lab.long(), weight = torch.Tensor([0.9,0.1])) + F.cross_entropy(edge_u.cpu(), edgeu_lab.long(), weight = torch.Tensor([0.9,0.1]))

            loss = (loss_l + loss_u) * (1-args.alpha) + loss_edge * args.alpha
            ...
            writer.add_scalar('Self/loss_edge', loss_edge, iter_num)
            writer.add_scalar('Self/loss_all', loss, iter_num)

            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            logging.info('iteration %d : loss: %03f, loss_l: %03f, loss_u: %03f, loss_edge: %03f'%(iter_num, loss, loss_l, loss_u, loss_edge))
...
if __name__ == "__main__":
    ## make logger file
    pre_snapshot_path = "./LA_result_{}_encoder/pretrain_{}_{}/pretrain".format(args.alpha, args.labelnum, args.pretrain_model)
    self_snapshot_path = "./LA_result_{}_encoder/pretrain_{}_{}/self_train_{}_{}".format(args.alpha, args.labelnum, args.pretrain_model, args.model, args.alpha)
